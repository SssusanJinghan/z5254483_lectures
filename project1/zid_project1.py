...
def get_tics(pth):
    ...
    with open(pth, mode = 'r') as fobj:
        ...
        for line in fobj:
            if line.strip():
                (exchange, tic) = line.strip().replace('\"','').split('=')
                exchange = exchange.lower()
                tic = tic.lower()
                tic_dict[tic] = exchange
    return tic_dict
...
def read_dat(tic):
    ...
    with open(file_path, mode = 'r') as fobj:
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        if tickers_lst == []:
            raise Exception("tickers_lst cannot be an empty list")
        for tic in tickers_lst:
            if tic not in list(tic_exchange_dic.keys()):
                raise Exception(f"ticker {tic} does not correspond to a key in tic_exchange_dic")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        if col_lst == []:
            raise Exception("col_lst cannot be an empty list")
        for col in col_lst:
            if col not in COLUMNS:
                raise Exception(f"Column {col} not found in COLUMNS")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        ...
        data = []
        for line in read_dat(tic):
            rec_dict = {}
            for col in col_lst:
                rec_dict[col] = line_to_dict(line)[col]
